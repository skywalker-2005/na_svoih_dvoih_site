...
const Oopt = () => {
  ...
  const filteredOopt = oopt.filter(oopt => {
    let filter1 = oopt.title.toLowerCase().includes(territoryName.toLowerCase()) || !territoryName;
    let filter2 = oopt.ooptType.toLowerCase().includes(ooptType.toLowerCase()) || !ooptType;
    let filter3 = oopt.region.toLowerCase().includes(region.toLowerCase()) || !region;
  ...
  let options_ooptType = ['Заповедник', 'Национальный парк'].map(item => {return {value: item, label: item}});
  let options_region = ['Краснодарский к.', 'р. Дагестан', 'р. Ингушетия', 'Кабардино-Балкарская р.', 'Карачаево-Черкесская р.', 'р. Северная Осетия - Алания', 'Ставропольский к.', 'Чеченская р.'].map(item => {return {value: item, label: item}});
  ...
  return (
    <>
      <div className="OoptSearchBar" style={{ backgroundImage: `url(${oopt_top})`, backgroundSize: 'cover',  backgroundPosition: 'center' }}>
        <div className="oopt_wrap">
          <div className="oopt_flex">
            ...
            <div className="input-group">
                <Select
                  ...
                  onChange={(selectedOption) => {
                    if (selectedOption.value === '')
                      ...
                      setTerritoryName(selectedOption.value);
                  }}
                ...
                <Select
                  ...
                  onChange={(selectedOption) => {
                    if (selectedOption.value === '')
                      ...
                      setOoptType(selectedOption.value);
                  }}
                ...
                onClick={() => {setTerritoryName(''); setOoptType(''); setRegion('')}} 
                >Сбросить</button>
            </div>
          </div>
        </div>
